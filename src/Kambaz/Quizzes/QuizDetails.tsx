import React, { useEffect, useState } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { getQuizById, publishQuiz, unpublishQuiz, createAttempt, getAttemptsForQuiz } from './api';
import { Container, Button, Card, Badge, ListGroup, Alert, Row, Col, Table } from 'react-bootstrap';
import { FaEdit, FaPlay, FaEye, FaCheckCircle, FaTimesCircle, FaCalendarAlt, FaLock, FaHistory } from 'react-icons/fa';
import './QuizDetails.css';
import { useSelector } from 'react-redux';

interface Quiz {
  _id: string;
  title: string;
  description: string;
  quizType: string;
  totalPoints: number;
  published: boolean;
  timeLimit: number;
  shuffleAnswers: boolean;
  multipleAttempts: boolean;
  attemptsAllowed: number;
  showCorrectAnswers: boolean;
  accessCode: string;
  oneQuestionAtTime: boolean;
  webcamRequired: boolean;
  lockQuestionsAfterAnswering: boolean;
  dueDate?: Date;
  availableDate?: Date;
  untilDate?: Date;
  assignmentGroup: string;
  questions: Array<{
    _id: string;
    title: string;
    questionType: string;
    questionText: string;
    points: number;
  }>;
}

function QuizDetails() {
  const { quizId } = useParams<{ quizId: string }>();
  const navigate = useNavigate();
  const { currentUser } = useSelector((state: any) => state.accountReducer);
  
  const [quiz, setQuiz] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [lastAttempt, setLastAttempt] = useState<any | null>(null);
  const [hasCompletedAttempt, setHasCompletedAttempt] = useState(false);
  
  // Determine current user role
  const isTeacher = currentUser && currentUser.role === 'FACULTY';
  const isStudent = currentUser && currentUser.role === 'STUDENT';

  useEffect(() => {
    if (!quizId) return;
    
    const fetchData = async () => {
      try {
        setLoading(true);
        
        // Get quiz details
        const response = await getQuizById(quizId);
        setQuiz(response.data);
        
        if (isStudent) {
          // Get student's quiz attempt records
          try {
            const attemptsData = await getAttemptsForQuiz(quizId);
            if (Array.isArray(attemptsData) && attemptsData.length > 0) {
              // Get the latest attempt
              const latestAttempt = attemptsData[0]; // Already sorted by creation time
              setLastAttempt(latestAttempt);
              
              if (latestAttempt.completed) {
                setHasCompletedAttempt(true);
              }
            }
          } catch (err: any) {
            console.error("Failed to get quiz attempt records:", err);
          }
        }
      } catch (err: any) {
        setError(err.message || "Failed to get quiz details");
      } finally {
        setLoading(false);
      }
    };
    
    fetchData();
  }, [quizId, isStudent]);

  const handleEditQuiz = () => {
    navigate(`/Kambaz/Quizzes/${quizId}/edit`);
  };

  const handlePreviewQuiz = () => {
    navigate(`/Kambaz/Quizzes/${quizId}/preview`);
  };

  const handlePublishToggle = async () => {
    if (!quiz) return;
    
    try {
      const response = quiz.published 
        ? await unpublishQuiz(quiz._id)
        : await publishQuiz(quiz._id);
      
      setQuiz(response.data);
    } catch (err: any) {
      setError(`${quiz.published ? 'Unpublish' : 'Publish'} quiz failed: ${err.message}`);
    }
  };

  const handleStartQuiz = async () => {
    if (!quiz) return;
    
    try {
      console.log(`Attempting to start quiz: ${quiz._id}`);
      const response = await createAttempt(quiz._id);
      
      // Ensure response and response.data exist and are valid
      if (!response || !response.data) {
        throw new Error("Received invalid attempt record response");
      }
      
      console.log(`Quiz attempt created successfully, entering attempt page, attempt ID=${response.data._id}`);
      navigate(`/Kambaz/Quizzes/${quiz._id}/attempt`, { 
        state: { attemptId: response.data._id } 
      });
    } catch (err: any) {
      console.error("Error starting quiz:", err);
      const errorMessage = err.response?.data?.message || err.message || "Unknown error";
      setError(`Failed to start quiz: ${errorMessage}`);
    }
  };

  // Add function to view most recent attempt results
  const handleViewResults = () => {
    if (lastAttempt && lastAttempt._id) {
      navigate(`/Kambaz/Quizzes/${quizId}/results`, {
        state: {
          attemptId: lastAttempt._id,
          score: lastAttempt.score,
          totalPoints: quiz?.totalPoints
        }
      });
    }
  };

  // Format date display
  const formatDate = (dateString?: Date) => {
    if (!dateString) return 'Not set';
    
    const date = new Date(dateString);
    return `${date.toLocaleDateString()} ${date.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}`;
  };

  if (loading) return (
    <Container className="text-center my-5">
      <div className="spinner-border" role="status">
        <span className="visually-hidden">Loading...</span>
      </div>
    </Container>
  );

  if (error) return (
    <Container className="my-5">
      <Alert variant="danger">
        <Alert.Heading>An Error Occurred</Alert.Heading>
        <p>{error}</p>
      </Alert>
    </Container>
  );

  if (!quiz) return (
    <Container className="my-5">
      <Alert variant="warning">
        <Alert.Heading>Quiz Not Found</Alert.Heading>
        <p>Unable to find the specified quiz, please return to the quiz list page.</p>
      </Alert>
    </Container>
  );

  const getQuizTypeLabel = (type: string) => {
    switch (type) {
      case 'GRADED_QUIZ': return 'Graded Quiz';
      case 'PRACTICE_QUIZ': return 'Practice Quiz';
      case 'GRADED_SURVEY': return 'Graded Survey';
      case 'UNGRADED_SURVEY': return 'Ungraded Survey';
      default: return type;
    }
  };

  const getAssignmentGroupLabel = (group: string) => {
    switch (group) {
      case 'QUIZZES': return 'Quizzes';
      case 'EXAMS': return 'Exams';
      case 'ASSIGNMENTS': return 'Assignments';
      case 'PROJECT': return 'Project';
      default: return group;
    }
  };

  return (
    <Container className="my-4 quiz-details-container">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="page-title">{quiz?.title}</h2>
        <div className="action-buttons">
          {isTeacher && (
            <>
              <Button 
                variant="outline-secondary" 
                className="me-2"
                onClick={() => navigate(`/Kambaz/Quizzes/${quizId}/preview`)}
              >
                <FaEye className="me-1" /> Preview
              </Button>
              <Button 
                variant="outline-primary" 
                className="me-2"
                onClick={() => navigate(`/Kambaz/Quizzes/${quizId}/edit`)}
              >
                <FaEdit className="me-1" /> Edit
              </Button>
            </>
          )}
          {isStudent && quiz?.published && (
            <>
              <Button 
                variant="success" 
                className="me-2"
                onClick={handleStartQuiz}
                disabled={quiz?.multipleAttempts === false && hasCompletedAttempt}
              >
                <FaPlay className="me-1" /> Start Quiz
              </Button>
              
              {hasCompletedAttempt && (
                <Button 
                  variant="primary" 
                  onClick={handleViewResults}
                >
                  <FaHistory className="me-1" /> View Results
                </Button>
              )}
            </>
          )}
        </div>
      </div>

      <Card className="quiz-details-card mb-4">
        <Card.Header className="quiz-card-header">
          <div className="d-flex justify-content-between">
            <div>
              <Badge bg={quiz.published ? "success" : "secondary"} className="me-2">
                {quiz.published ? "Published" : "Unpublished"}
              </Badge>
              <Badge bg="info">{getQuizTypeLabel(quiz.quizType || 'GRADED_QUIZ')}</Badge>
            </div>
            <div>
              <Badge bg="primary">{quiz.totalPoints || 0} points</Badge>
            </div>
          </div>
        </Card.Header>
        <Card.Body>
          {quiz.description && (
            <Card.Text>{quiz.description}</Card.Text>
          )}

          <div className="quiz-properties">
            <h4 className="section-title">Quiz Settings</h4>
            
            <Table striped bordered responsive className="property-table">
              <tbody>
                <tr>
                  <td className="property-name">Quiz Type</td>
                  <td>{getQuizTypeLabel(quiz.quizType || 'GRADED_QUIZ')}</td>
                </tr>
                <tr>
                  <td className="property-name">Total Points</td>
                  <td>{quiz.totalPoints || 0} points</td>
                </tr>
                <tr>
                  <td className="property-name">Assignment Group</td>
                  <td>{getAssignmentGroupLabel(quiz.assignmentGroup || 'QUIZZES')}</td>
                </tr>
                <tr>
                  <td className="property-name">Shuffle Answers</td>
                  <td>
                    {quiz.shuffleAnswers ? 
                      <Badge bg="success">Yes</Badge> : 
                      <Badge bg="secondary">No</Badge>
                    }
                  </td>
                </tr>
                <tr>
                  <td className="property-name">Time Limit</td>
                  <td>{quiz.timeLimit || 20} minutes</td>
                </tr>
                <tr>
                  <td className="property-name">Allow Multiple Attempts</td>
                  <td>
                    {quiz.multipleAttempts ? 
                      <Badge bg="success">Yes ({quiz.attemptsAllowed || 1} times)</Badge> : 
                      <Badge bg="secondary">No</Badge>
                    }
                  </td>
                </tr>
                <tr>
                  <td className="property-name">Show Correct Answers</td>
                  <td>
                    {quiz.showCorrectAnswers ? 
                      <Badge bg="success">Yes</Badge> : 
                      <Badge bg="secondary">No</Badge>
                    }
                  </td>
                </tr>
                <tr>
                  <td className="property-name">Access Code</td>
                  <td>{quiz.accessCode ? quiz.accessCode : <span className="text-muted">None</span>}</td>
                </tr>
                <tr>
                  <td className="property-name">One Question at a Time</td>
                  <td>
                    {quiz.oneQuestionAtTime ? 
                      <Badge bg="success">Yes</Badge> : 
                      <Badge bg="secondary">No</Badge>
                    }
                  </td>
                </tr>
                <tr>
                  <td className="property-name">Require Webcam</td>
                  <td>
                    {quiz.webcamRequired ? 
                      <Badge bg="success">Yes</Badge> : 
                      <Badge bg="secondary">No</Badge>
                    }
                  </td>
                </tr>
                <tr>
                  <td className="property-name">Lock Questions After Answering</td>
                  <td>
                    {quiz.lockQuestionsAfterAnswering ? 
                      <Badge bg="success">Yes</Badge> : 
                      <Badge bg="secondary">No</Badge>
                    }
                  </td>
                </tr>
              </tbody>
            </Table>

            <h4 className="section-title mt-4">Time Settings</h4>
            
            <Table striped bordered responsive className="property-table">
              <tbody>
                <tr>
                  <td className="property-name">Due Date</td>
                  <td>{formatDate(quiz.dueDate)}</td>
                </tr>
                <tr>
                  <td className="property-name">Available From</td>
                  <td>{formatDate(quiz.availableDate)}</td>
                </tr>
                <tr>
                  <td className="property-name">Available Until</td>
                  <td>{formatDate(quiz.untilDate)}</td>
                </tr>
              </tbody>
            </Table>

            <h4 className="section-title mt-4">Quiz Questions ({quiz.questions.length || 0})</h4>
            {quiz.questions.length === 0 ? (
              <Alert variant="warning">This quiz has no questions yet</Alert>
            ) : (
              <ListGroup className="question-list">
                {quiz.questions.map((question: Quiz['questions'][0], index: number) => (
                  <ListGroup.Item key={question._id} className="question-item">
                    <div className="d-flex justify-content-between">
                      <div>
                        <span className="question-number">Question {index + 1}:</span> {question.title}
                        <div className="question-text">{question.questionText}</div>
                      </div>
                      <Badge bg="primary" className="question-points">{question.points} points</Badge>
                    </div>
                  </ListGroup.Item>
                ))}
              </ListGroup>
            )}
          </div>
        </Card.Body>
      </Card>
    </Container>
  );
}

export default QuizDetails;